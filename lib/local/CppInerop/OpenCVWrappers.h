...
namespace OpenCVWrappers {
	...
	public:
		...
		RawImage(Emgu::CV::Mat^ m)
		{
			mat = new cv::Mat(reinterpret_cast<cv::Mat*>(m->Ptr.ToPointer())->clone());
		}
		...
		RawImage(System::Drawing::Bitmap^ bitmap)
		{
			...
			else if (fmt == System::Drawing::Imaging::PixelFormat::Format8bppIndexed)
			{
				mat = new cv::Mat(cv::Size(bitmap->Width, bitmap->Height), CV_8UC1);
			}
			else
			{
				throw gcnew System::Exception("Unsupported image type");
