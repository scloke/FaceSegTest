...
{
	cv::Mat my_mask = postprocessmask(out, ori_image, h_in, w_in, re, seg_largest_only, blur_r);
...
cv::Mat model::postprocessmask(cv::Mat out, cv::Mat ori_image, int h_in, int w_in, bool re, bool seg_largest_only, int blur_r)
{
	// get mask with type CV_8U
	...
	// remove holes at base of mask
	auto baseroi = cv::Rect(0, h_in - 2, w_in, 2);
	cv::dilate(my_mask(baseroi), my_mask(baseroi), cv::Mat(), cv::Point(-1, -1), 1, cv::BORDER_CONSTANT, 255);

	auto cf = cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(blur_r, blur_r));
	cv::morphologyEx(my_mask, my_mask, cv::MORPH_OPEN, cf, cv::Point(-1, -1), 1, cv::BORDER_ISOLATED);
	cv::morphologyEx(my_mask, my_mask, cv::MORPH_CLOSE, cf, cv::Point(-1, -1), 1, cv::BORDER_ISOLATED);
	...
	if (my_mask.type() != CV_8U) {
		my_mask.convertTo(my_mask, CV_8U);
	}

	if (re)
	{
		cv::resize(my_mask, my_mask, cv::Size(w, h), 0.0, 0.0, cv::INTER_NEAREST);
	}

	cv::GaussianBlur(my_mask, my_mask, cv::Size(blur_r, blur_r), 7.0, 0.0, cv::BORDER_ISOLATED);
